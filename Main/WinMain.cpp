#include "Startup/ImGuiManager.h"
...
{
	//This will be our browser for use of scraping
	BrowserManager GLRBrowser;
	
	//Create the GLRManager that handles all the main functions
	GLRManager GLRManager;

    //Setup the ImGui and Initialize it with the necessary information, if it doesn't launch something happened here.
    ImGuiOpenGL ImguiManager;
    if (ImguiManager.InitImgui(GLRManager.GetProgramName() + " " + GLRManager.GetProgramVersion()) == false)
        return 0;


	//Variables used within the main logic loops
	...
	//Variables used within the loops (For Profile)
	...
	bool AlsoAddGames = false;
	bool BeginSearch = true;
	int SearchWaitLoops = 0;
	
	//The Search Terms used to find our games
    ...
    while (!glfwWindowShouldClose(ImguiManager.GetWindow()))
    {
        ...
        if (GLRManager.GetGreenlumaPath().empty())
        {
            ...
            if (ImGui::BeginPopupModal("Find Greenluma Path", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysAutoResize))
            {
                ...
                ImGui::Text("There is no path associated for Greenluma, please locate your Greenluma path. (ie. Where your DLLInjector is)");
                ImGui::Separator();
                ImGui::Spacing();
            ...
            if (BeginSearch)
            {
                if (ImGui::BeginPopupModal("Searching", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysAutoResize))
                {
                    ...
                    if (SearchWaitLoops > 8)
                    {
                        ...
                        GLRBrowser.SearchSteamGames(SearchTerms);
